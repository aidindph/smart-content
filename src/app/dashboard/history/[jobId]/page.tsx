...
export default async function JobDetailsPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      {active ? <GenerationProgress imageCount={request.image_count} job={job} preview={job.output_markdown} steps={steps ?? []} targetWords={request.target_words} /> : null}
      ...
      {job.status === "completed" && job.output_markdown ? <section className="panel mt-6 p-6"><div className="flex flex-wrap items-center justify-between gap-4"><h2 className="text-xl font-black">متن نهایی</h2><CopyButton label="کپی متن" value={job.output_markdown} /></div><article className="mt-6 whitespace-pre-wrap leading-8">{job.output_markdown}</article></section> : null}
    </div>
  );
}
