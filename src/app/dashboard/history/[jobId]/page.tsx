...
export default async function JobDetailsPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      {job.output_markdown ? <section className="panel mt-6 p-6"><div className="flex flex-wrap items-center justify-between gap-4"><h2 className="text-xl font-black">متن نهایی</h2><span className="text-xs text-muted">قالب مارک‌داون</span></div><article className="mt-6 whitespace-pre-wrap leading-8">{job.output_markdown}</article></section> : null}
    </div>
  );
}
