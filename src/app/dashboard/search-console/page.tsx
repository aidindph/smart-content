...
type KeyRow = { id: string; provider_id: string; label: string; key_hint: string; default_text_model: string | null };
...
export default async function SearchConsolePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const results = await Promise.all([
    ...
    supabase.from("user_api_keys").select("id, provider_id, label, key_hint, default_text_model").is("deleted_at", null).eq("is_active", true).eq("test_status", "valid").returns<KeyRow[]>(),
    admin.from("system_api_keys").select("id, provider_id, label, key_hint, default_text_model").is("deleted_at", null).eq("is_active", true).eq("test_status", "valid").returns<KeyRow[]>(),
    supabase.from("providers").select("id, name, slug, enabled").eq("enabled", true).returns<Provider[]>(),
  ]);
  const [connection, properties, suggestions, metrics, sitemaps, runs, keys, systemKeys, providers] = results.map((result) => result.data) as [Connection | null, Property[] | null, Suggestion[] | null, Metric[] | null, Sitemap[] | null, SyncRun[] | null, KeyRow[] | null, KeyRow[] | null, Provider[] | null];
