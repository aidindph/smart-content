...
export default async function SearchConsolePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [{ data: connection }, { data: properties }, { data: suggestions }, { data: metrics }, { data: keys }, { data: providers }] = await Promise.all([
  ...
  const keyOptions = (keys ?? []).flatMap((key) => { const provider = providerById.get(key.provider_id); return provider ? [{ ...key, provider }] : []; });
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <header><p className="text-sm font-bold text-brand">فرصت‌های محتوایی</p><h1 className="mt-2 text-3xl font-black">گوگل سرچ کنسول</h1><p className="mt-3 max-w-3xl leading-7 text-muted">عملکرد جست‌وجو را همگام کنید و با تکیه بر نمایش، نرخ کلیک و جایگاه واقعی، عنوان تازه بسازید.</p></header>
