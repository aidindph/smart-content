...
export function SuggestTitleForm({ properties, keys, models }: { properties: Property[]; keys: KeyOption[]; models: Model[] }) {
  ...
  const preferredModel = availableModels.find((model) => model.model_key === "gemini-2.5-flash")
    ?? availableModels.find((model) => model.model_key === "gemini-flash-latest")
    ?? availableModels[0];
  ...
  return <form action={suggestTitlesAction} className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_auto]">
    ...
    <label className="grid gap-2 text-sm font-bold">مدل نگارش<select className="field" defaultValue={preferredModel?.model_key} dir="ltr" key={connectionId} name="model" required>{availableModels.map((model) => <option key={model.model_key} value={model.model_key}>{model.display_name}</option>)}</select></label>
