...
import { deleteSystemApiKeyAction, toggleSystemApiKeyAction } from "./system-api-key-actions";
import { SystemApiKeyForm } from "./system-api-key-form";
...
type SystemKey = { id: string; provider_id: string; label: string; key_hint: string; is_active: boolean; test_status: "valid" | "invalid" | "untested"; last_tested_at: string | null };

export default async function SystemAdminPage() {
  const { supabase, profile: currentProfile } = await requireSystemAdmin();
  const admin = createAdminClient();
  const [settingsResult, providersResult, profilesResult, quotaResult, invitationsResult, jobsResult, authUsersResult, modelsResult, pricingResult, systemKeysResult] = await Promise.all([
    ...
    admin.from("system_api_keys").select("id, provider_id, label, key_hint, is_active, test_status, last_tested_at").is("deleted_at", null).returns<SystemKey[]>(),
  ]);
  ...
  const systemKeys = systemKeysResult.data ?? [];
  ...
  return (
    <main className="admin-shell">
      <aside className="admin-sidebar">
        <Link className="admin-brand" href="/system-admin"><span><AppIcon name="shield" /></span><div><strong>مرکز فرمان</strong><small>مدیریت موتور محتوا</small></div></Link>
        <nav><a href="#overview"><AppIcon name="activity" />نمای کلی</a><a href="#ai"><AppIcon name="server" />موتورهای هوش مصنوعی</a><a href="#models"><AppIcon name="settings" />مدل‌ها و قیمت‌ها</a><a href="#users"><AppIcon name="users" />کاربران و سهمیه‌ها</a><a href="#access"><AppIcon name="key" />دسترسی و دعوت</a></nav>
        <div className="admin-system-state"><i className={settings.maintenance_mode ? "bg-amber-400" : "bg-emerald-400"} /><div><strong>{settings.maintenance_mode ? "حالت تعمیر" : "سامانه عملیاتی است"}</strong><small>{settings.registration_enabled ? "ثبت‌نام عمومی باز" : "ثبت‌نام عمومی بسته"}</small></div></div>
        <Link className="admin-back" href="/dashboard"><AppIcon name="arrow" />بازگشت به داشبورد کاربر</Link>
      </aside>
      <div className="admin-workspace">
        <header className="admin-header" id="overview"><div><span>پنل اختصاصی مدیر سیستم</span><h1>سلام، {currentProfile.display_name ?? "مدیر سامانه"}</h1><p>سلامت سرویس‌ها، دسترسی کاربران و موتورهای تولید را از این مرکز کنترل کنید.</p></div><div className="admin-date"><AppIcon name="activity" /><span>وضعیت لحظه‌ای<strong>{new Intl.DateTimeFormat("fa-IR", { dateStyle: "long" }).format(new Date())}</strong></span></div></header>

        <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <article className="admin-stat violet"><span><AppIcon name="users" /></span><div><p>کاربران</p><strong>{new Intl.NumberFormat("fa-IR").format(profiles.length)}</strong><small>حساب ثبت‌شده</small></div></article>
          <article className="admin-stat cyan"><span><AppIcon name="activity" /></span><div><p>کل اجراها</p><strong>{new Intl.NumberFormat("fa-IR").format(jobs.length)}</strong><small>در تمام زمان‌ها</small></div></article>
          <article className="admin-stat emerald"><span><AppIcon name="check" /></span><div><p>تکمیل‌شده</p><strong>{new Intl.NumberFormat("fa-IR").format(completedCount)}</strong><small>خروجی موفق</small></div></article>
          <article className="admin-stat rose"><span><AppIcon name="zap" /></span><div><p>ناموفق</p><strong>{new Intl.NumberFormat("fa-IR").format(failedCount)}</strong><small>نیازمند بررسی</small></div></article>
        </section>

        <section className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_.85fr]" id="ai">
          <SystemApiKeyForm providers={providers.map(({ id, name }) => ({ id, name }))} />
          <article className="admin-card p-6"><div className="admin-section-title"><span className="admin-icon bg-cyan-500/15 text-cyan-300"><AppIcon name="server" /></span><div><small>اتصال‌های فعال</small><h2>کلیدهای سراسری</h2></div></div>{systemKeys.length ? <div className="mt-5 grid gap-3">{systemKeys.map((key) => { const provider = providers.find((item) => item.id === key.provider_id); return <div className="admin-key-row" key={key.id}><div><strong>{provider?.name ?? key.label}</strong><small>{key.label} · <span dir="ltr">{key.key_hint}</span></small></div><span className={key.is_active ? "admin-status active" : "admin-status"}>{key.is_active ? "فعال" : "خاموش"}</span><div className="flex gap-2"><form action={toggleSystemApiKeyAction}><input name="keyId" type="hidden" value={key.id} /><input name="active" type="hidden" value={String(!key.is_active)} /><button className="admin-ghost" type="submit">{key.is_active ? "خاموش" : "روشن"}</button></form><form action={deleteSystemApiKeyAction}><input name="keyId" type="hidden" value={key.id} /><button className="admin-danger" type="submit">حذف</button></form></div></div>; })}</div> : <div className="admin-empty"><AppIcon name="key" /><strong>هنوز کلید سراسری ندارید</strong><p>فرم روبه‌رو را کامل کنید تا اتصال در داشبورد تمام کاربران ظاهر شود.</p></div>}</article>
        </section>

        <section className="mt-6 grid gap-6 xl:grid-cols-3" id="access">
